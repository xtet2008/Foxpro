...
* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ...
 ENDIF 
 _SCREEN.CAPTION = 'International Trader v8.1.1 xp'
 SET DELETED ON
 ...
 IF SETPATH()
   ...
   ALLTRIM(PARAMETE.COMPANY2)
    ...
    STORE ALLTRIM(PARAMETE.PRICE_PICT) TO PRICE_PICT
    ...
    QTY_DECIMAL = 2
    TOTAL_DECIMAL = 2
    IF AT('.',ALLTRIM(PRICE_PICT)) = 0
       PRICE_DECIMAL = 0
    ELSE 
       PRICE_DECIMAL = LEN(ALLTRIM(PRICE_PICT)) - AT('.',ALLTRIM(PRICE_PICT))
    ENDIF 
    ...
    IF  .NOT. EMPTY(PARAMETE.PRICE_PICT)
    ...
    STORE ALLTRIM(PARAMETE.PRICE_PICT) TO BOMPRICE_PICT
    ...
    IF  .NOT. EOF()
       STORE ALLTRIM(PARAMETE.PRICE_PICT) TO WT_PICT
       ...
       IF AT('.',ALLTRIM(PRICE_PICT)) = 0
          WT_DECIMAL = 0
       ELSE 
          WT_DECIMAL = LEN(ALLTRIM(PRICE_PICT)) - AT('.',ALLTRIM(PRICE_PICT))
       ENDIF 
       IF AT('.',ALLTRIM(QTY_PICT)) = 0
          CBM_DECIMAL = 0
       ELSE 
